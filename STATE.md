...
- **Manual Question Selection for Exams complete (Phases 1-3, see DONE)** — remaining: verify drag-drop cross-section + PDF with Myanmar template on real data (needs a multi-question bank).
...
7. **Exam module**: full drag-drop reorder verification (within + between sections) and PDF render of multi-section exam with Myanmar template on real multi-question data
